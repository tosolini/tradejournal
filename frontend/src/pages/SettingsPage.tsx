  ...
  return (
    <section className="card border border-teal-500/40 bg-teal-500/5 p-4">
      <div className="mb-3 flex items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-teal-200 dark:text-teal-700">{t("settings.onboarding.title")}</h2>
          <p className="mt-1 text-sm text-slate-400 dark:text-slate-500 dark:text-slate-400">{t("settings.onboarding.description")}</p>
        </div>
        <button
          onClick={onDismiss}
      ...
      <ol className="space-y-2 text-sm">
        <li className="flex items-center gap-2">
          ...
          <span className="text-slate-300 dark:text-slate-700">{t("settings.onboarding.step_login")}</span>
        </li>
        <li className="flex items-center gap-2">
          <span className="flex h-5 w-5 items-center justify-center rounded-full border border-slate-500 text-xs font-bold text-slate-400 dark:text-slate-500 dark:text-slate-400">2</span>
          <span className="text-slate-300 dark:text-slate-700">{t("settings.onboarding.step_credentials")}</span>
        </li>
        <li className="flex items-center gap-2">
          <span className="flex h-5 w-5 items-center justify-center rounded-full border border-slate-500 text-xs font-bold text-slate-400 dark:text-slate-500 dark:text-slate-400">3</span>
        ...
        <li className="flex items-center gap-2">
          <span className="flex h-5 w-5 items-center justify-center rounded-full border border-slate-500 text-xs font-bold text-slate-400 dark:text-slate-500 dark:text-slate-400">4</span>
          <span className="text-slate-300 dark:text-slate-700">
...
function AccountSection({ user }: { user: User }) {
  ...
  return (
    <section className="card p-4">
      ...
      <p className="mb-4 text-sm text-slate-400 dark:text-slate-500 dark:text-slate-400">{t("settings.account.description")}</p>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <label className="mb-1 block text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400">{t("settings.account.username")}</label>
            <input
              value={username}
              onChange={(e) => setUsername(e.target.value)}
              className="w-full rounded border border-slate-700 dark:border-slate-300 bg-slate-900 dark:bg-white px-3 py-2 text-sm text-slate-200"
            />
          </div>
          <div>
            <label className="mb-1 block text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400">{t("settings.account.email")}</label>
            <input
              type="email"
              value={email}
        ...
        <div className="border-t border-slate-700/60 dark:border-slate-300/60 pt-4">
          <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500 dark:text-slate-400">{t("settings.account.change_password")}</p>
          <div className="grid gap-4 md:grid-cols-3">
            <div>
              <label className="mb-1 block text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400">{t("settings.account.current_password")}</label>
              <input
                type="password"
            ...
            <div>
              <label className="mb-1 block text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400">{t("settings.account.new_password")}</label>
              <input
                type="password"
                value={newPassword}
            ...
            <div>
              <label className="mb-1 block text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400">{t("settings.account.confirm_password")}</label>
              <input
                type="password"
        ...
        <div className="flex items-center justify-between">
          <div className="text-xs text-slate-500 dark:text-slate-400">
            {t("settings.account.role_label")}: <span className="font-medium text-slate-300 dark:text-slate-700">{user.role}</span>
          </div>
          <button
            type="submit"
        ...
        <div className="space-y-3">
          <div>
            ...
            <input
              value={username}
              onChange={(e) => setUsername(e.target.value)}
              className="w-full rounded border border-slate-700 dark:border-slate-300 bg-slate-800 dark:bg-slate-100 px-3 py-2 text-sm text-slate-200"
            />
          </div>
          <div>
            <label className="mb-1 block text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400">{t("settings.account.email")}</label>
          ...
          <div>
            <label className="mb-1 block text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400">
          ...
          <div>
            <label className="mb-1 block text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400">{t("settings.account.role_label")}</label>
            <select
              value={role}
    ...
    <section className="card p-4">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500 dark:text-slate-400">{t("settings.admin_users.description")}</p>
        </div>
        <button
          ...
          <tbody>
            {users.map((u) => (
              <tr key={u.id} className="border-b border-slate-800/80 hover:bg-slate-800/40">
                <td className="px-3 py-2 font-medium text-teal-200 dark:text-teal-700">
                  {u.username}
                  {u.id === currentUser.id && (
                    <span className="ml-2 rounded-full bg-teal-500/20 px-1.5 py-0.5 text-[10px] text-teal-300 dark:text-teal-700">
                      {t("settings.admin_users.you")}
                    </span>
                  )}
                </td>
                <td className="px-3 py-2 text-slate-300 dark:text-slate-700">{u.email}</td>
                <td className="px-3 py-2">
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                    u.role === "admin"
                      ? "bg-amber-500/20 text-amber-300"
                      : "bg-slate-700/60 text-slate-400 dark:text-slate-500 dark:text-slate-400"
                  }`}>
                    {u.role}
                  </span>
                </td>
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="card p-4">
        ...
        <p className="mb-4 text-sm text-slate-400 dark:text-slate-500 dark:text-slate-400">{t("settings.language.description")}</p>
      ...
      <section className="card p-4">
        ...
        <p className="mb-4 text-sm text-slate-400 dark:text-slate-500 dark:text-slate-400">{t("settings.registry.description")}</p>
        <Link
          to="/settings/brokers"
      ...
      <section className="card p-4">
        ...
        <p className="mb-4 text-sm text-slate-400 dark:text-slate-500 dark:text-slate-400">{t("settings.session.description")}</p>
        <button
          onClick={() => {
