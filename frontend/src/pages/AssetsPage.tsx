    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
        ...
        <div className="space-y-3">
          <div>
            <label className="mb-1 block text-xs text-slate-400">{t("assets.symbol")}</label>
            <input
              value={symbol}
          ...
          <div>
            <label className="mb-1 block text-xs text-slate-400">{t("assets.name")}</label>
            <input
              value={name}
          ...
          <div>
            <label className="mb-1 block text-xs text-slate-400">{t("assets.isin")}</label>
            <input
              value={isin}
          ...
          <div>
            <label className="mb-1 block text-xs text-slate-400">{t("assets.instrument_type")}</label>
            <select
          ...
          <div>
            <label className="mb-1 block text-xs text-slate-400">{t("assets.exchange")}</label>
            <input
              value={exchange}
          ...
          <div>
            <label className="mb-1 block text-xs text-slate-400">{t("assets.currency")}</label>
            <input
              value={currency}
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400">{t("assets.subtitle")}</p>
        </div>
        <button
          ...
          <tbody>
            ...
            {(assets ?? []).map((asset: Asset) => (
              <tr key={asset.id} className="border-b border-slate-800/80 hover:bg-slate-800/40">
                <td className="px-3 py-2 font-medium text-teal-200">{asset.symbol}</td>
                <td className="px-3 py-2 text-slate-300">{asset.name}</td>
                <td className="px-3 py-2 font-mono text-xs text-slate-400">{asset.isin ?? "—"}</td>
                <td className="px-3 py-2 text-slate-400">{asset.instrument_type}</td>
                <td className="px-3 py-2 text-slate-400">{asset.exchange ?? "—"}</td>
                <td className="px-3 py-2 text-slate-400">{asset.currency}</td>
